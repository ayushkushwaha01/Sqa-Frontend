...
export class PauditsActionsComponent implements OnInit {
  ...
  loadLookups() {
    ...
    this.api.getEscalations().pipe(catchError(() => of({ success: false, data: [] }))).subscribe((res: any) => {
      ...
      this.api.getLookups().pipe(catchError(() => of({ success: false, data: [] }))).subscribe((lookupRes: any) => {
        if (lookupRes && lookupRes.success && Array.isArray(lookupRes.data)) {
          this.capaStatusLookups = lookupRes.data.filter((l: any) => l.codeMasterName === 'Capa-Status');
        }
        this.loadData();
      });
