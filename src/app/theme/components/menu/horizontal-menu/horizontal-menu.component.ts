...
import { supplierMenuItems } from "./../menu";
import {
  Component,
  OnInit,
  Input,
...
export class HorizontalMenuComponent implements OnInit {
  ...
  // ADDED: reads UserType from the JWT instead of localStorage,
  // since UserType is no longer written to localStorage.
  private getUserTypeFromToken(): string | null {
    const token = localStorage.getItem("jwt_token") || sessionStorage.getItem("jwt_token");
    if (!token) return null;
    try {
      const decoded: any = jwtDecode(token);
      return decoded.UserType || decoded.userType || null;
    } catch {
      return null;
    }
  }
  ...
  ngOnInit() {
    const isClient = localStorage.getItem("isClient");
    const userType = this.getUserTypeFromToken(); // CHANGED: was localStorage.getItem("UserType")
